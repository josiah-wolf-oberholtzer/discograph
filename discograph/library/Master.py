...
class Master(MongoModel, mongoengine.Document):
    ...
    def from_element(cls, element):
        ...
        if artists is not None and len(artists):
            artists = [library.ArtistCredit.from_element(_) for _ in artists]
        else:
            artists = None
        # data quality
        ...
        if main_release is not None:
            ...
            main_release = library.Release.objects.get(discogs_id=main_release)
        # release_date
        release_date = element.find('year')
        if release_date is not None:
            release_date = library.Release.parse_release_date(release_date.text)
        # styles
